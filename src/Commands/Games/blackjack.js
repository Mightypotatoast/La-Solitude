...
module.exports = {
    ...
    async execute(message, client) {
        ...
        let inviteRow = new MessageActionRow()
            .addComponents(
                new MessageButton()
                    .setLabel('Oui')
        ...
        let blackjackRow = new MessageActionRow()
            .addComponents(
        ...
        acceptCollector.on('collect', async (btn) => {
            ...
            if (btn.user.id !== message.member.id) {
                return await btn.reply({
                    embeds: [errorEmbed().setDescription('Vous ne pouvez pas intéragir car ce n\'est pas votre partie')],
                    ephemeral: true
                })
            }
            ...
            const blackjackCollector = blackjack.createMessageComponentCollector({
                type: 'BUTTON',
                time: 300000
            })
            ...
            blackjackCollector.on('collect', async (btn) => { 
                ...
                if (btn.customId === 'BJ-draw') {
                    ...
                    blackjackEmbed.fields[0].value = "" + blackjackEmbed.fields[0].value.slice(0, -2)


                    blackjackEmbed.fields[1].name = `- ( ${Score(playerHand)} ) - SCORE - ( ${Score(botHand)} ) -`
                ...
                switch (reason) { 
                    ...
                    case "stay":
                        ...
                        while (Score(botHand) < 17) {
                            ...
                            blackjackEmbed.fields[2].value = "" + blackjackEmbed.fields[2].value.slice(0, -2)
                        ...
                        if (Score(botHand) > 21) {
                            blackjackEmbed.fields[1].value = "Vous avez gagné !"
